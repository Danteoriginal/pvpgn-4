...
namespace pvpgn
{
	...
	namespace bnetd
	{
		...
		static int _client_adclick2(t_connection * c, t_packet const *const packet)
		{
			...
			eventlog(eventlog_level_trace, __FUNCTION__, "[%d] ad click2 for adid 0x%04hx from \"%s\"", conn_get_socket(c), bn_int_get(packet->u.client_adclick2.adid), conn_get_username(c)); conn_get_username(c));
